...
public class EntityRenderPostListenerImpl {
	...
	private static EntityRenderPostListener active = ctx -> { };

	public static void register(EntityRenderPostListener listener) {
		...
		if (LISTENERS.size() == 1) {
			...
			active = ctx -> {
				...
				for (int i = 0; i < limit; ++i) {
					LISTENERS.get(i).afterEntityRender(ctx);
				}
			};
		}
	}
