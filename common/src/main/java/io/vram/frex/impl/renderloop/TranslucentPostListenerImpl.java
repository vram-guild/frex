...
public class TranslucentPostListenerImpl {
	...
	private static TranslucentPostListener active = ctx -> { };

	public static void register(TranslucentPostListener listener) {
		...
		if (LISTENERS.size() == 1) {
			...
			active = ctx -> {
				...
				for (int i = 0; i < limit; ++i) {
					LISTENERS.get(i).afterTranslcuentRender(ctx);
				}
			};
		}
	}
	...
	public static void invoke(WorldRenderContext ctx) {
		active.afterTranslcuentRender(ctx);
	}
}
